...
s = df.groupby('대륙')['맥주'].mean().idxmax()
# print(s)
# 2) 1번에서 구한 대륙에서 다섯번째로 맥주 소비량이 많은 국가를 구하시오.
df2 = df.loc[df['대륙'] == s, ['국가','맥주']].set_index('국가').copy()
country = df2.sort_values(by='맥주', ascending=False).head(5).idxmin().iloc[0]
# 3) 2번에서 구한 국가의 맥주 소비량을 정수로 작성하시오.
# print(df2[df2.index==country]['맥주'].iloc[0]) # 313
...
a = df.sort_values('관광객비율', ascending=False).head(2)['관광'].iloc[1]
# - 2) 관광객 수가 두 번째로 높은 나라의 '공무'수를 b라고 정의하시오.
b = df.sort_values('관광', ascending=False).head(2)['공무'].iloc[1]
...
s1 = df['co']
s2 = df['nmch']
co_scaled = (s1 - s1.min()) / (s1.max() - s1.min())
nmch_scaled = (s2 - s2.min()) / (s2.max() - s2.min())
# 2) Min-Max Scale이 적용된 각 칼럼의 표준편차를 구하시오.
#   > co칼럼의 표준편차 = a, nmch 칼럼의 표준편차 = b
a = co_scaled.std(ddof=1)
b = nmch_scaled.std(ddof=1)
...
# 2023년 매출 증가율이 가장 높은 점포의 2022년총매출과 2023년의 총매출의 합을 정수로 출력한다.
