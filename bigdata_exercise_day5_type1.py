...
result = df[((df['tip'] / df['total_bill']) > 0.15) & (df['size'] >= 3)]['size'].sum()
# print(result) # 143
...
maxPassengers = df['passengers'].max()
# print(int(year1 + year2 + maxPassengers)) # 4541
...
word_bin = df['Cabin'].mode()[0]
df['Cabin'] = df['Cabin'].fillna(word_bin)
df = df[df['Cabin'] == word_bin]
age_mean = df.groupby(['Embarked','Pclass'])['Age'].transform('mean')
df['Age'] = df['Age'].fillna(age_mean)
age_median = df.groupby(['Embarked','Pclass'])['Age'].transform('median')
result = ((df['Age'] >= age_median) & (df['Fare'] > df['Fare'].median())).sum()
# print(result) # 194
...
formula = 'sales ~ ' + ' + '.join(train.columns[1:-1])
# print(formula)
...
# print((model.pvalues[1:] <= 0.05)) # TV, radio
...
print(round(temp.corr().loc['y_true', 'y_pred'], 3)) # 0.983

# 5-10) train 데이터를 사용하여 해당 모델의 예측값과 실제값의 스피어만(spearman) 상관계수를 구하여라.
# 결과는 반올림하여 소수점 아래 3자리까지 출력한다.
temp = pd.DataFrame({'y_true': train['sales'],
                     'y_pred': model2.predict(train)})
# print(round(temp.corr('spearman').loc['y_true','y_pred'], 3)) # 0.994
